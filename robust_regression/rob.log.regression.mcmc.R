# Function for running a robust logarithmic regression model in Stan
...
rob.log.regression.mcmc = function(x, y, x.pred = NULL, x.cred = NULL, pred.int = 90, cred.int = 95, 
    ...
    generated quantities {
        // Sample R-squared (explained variance)
        // This R2 incorporates the additional uncertainty in estimating the model 
        // parameters, and thus acts like an adjusted R2 (Gelman & Pardoe, 2006)
        //real<lower=0, upper=1> R2 = 1 - (dot_self(y - mu) / dot_self(y - mean(y)));
        real R2 = variance(mu) / variance(y);
    ...
    # Run the model
    stan.reg = stan(model_name="robust_log_regression",
    ...
    hpd.R2$`95` = HPDinterval(as.mcmc(as.numeric(stan.R2)), prob=0.95)
    hpd.R2$`99` = HPDinterval(as.mcmc(as.numeric(stan.R2)), prob=0.99)
    ...
    lines(x=x.cred, y=colMeans(stan.cred), lwd=3, col="grey30")
    ...
    cat("SIMPLE LOGARITHMIC REGRESSION: POSTERIOR STATISTICS\n",
        "Model: Y ~ t(alpha + beta * log(X), sigma, nu)\n\n",
        "Intercept (alpha)\n",
        ...
        "   Posterior mean and standard deviation: Mean = ", mean(stan.R2), ", SD = ", sd(stan.R2), "\n",
        "   Posterior median and MAD:              Median = ", median(stan.R2), ", MAD = ", mad(stan.R2), "\n",
        "   Values with 95% posterior probability: 95% HPDI = [", hpd.R2$`95`[,"lower"], ", ", hpd.R2$`95`[,"upper"], "]\n",
        "   Values with 99% posterior probability: 99% HPDI = [", hpd.R2$`99`[,"lower"], ", ", hpd.R2$`99`[,"upper"], "]\n\n",
        sep="")
